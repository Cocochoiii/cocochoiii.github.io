...
export default function HomePage({ go }) {
  ...
  useEffect(() => {
    ...
    const fn = (e) => {
      ...
      gsap.to(parallaxRef.current, { x: (e.clientX / window.innerWidth - 0.5) * 25, y: (e.clientY / window.innerHeight - 0.5) * 25, duration: 1.2, ease: 'power2.out' })
    }
  ...
  useEffect(() => {
    ...
    import('@rive-app/canvas').then((rive) => {
      ...
      const r = new rive.Rive({
        ...
        onLoad: () => { r.resizeDrawingSurfaceToCanvas(); const inputs = r.stateMachineInputs('State Machine 1'); const roll = inputs?.find((i) => i.name === 'roll'); if (roll) setTimeout(() => { roll.value = true }, 800) },
      })
      riveRef.current = r
    })
  ...
  const toggleMusic = useCallback(() => {
    ...
    if (musicOn) { bgMusic.pause(); setMusicOn(false); setDialogue('Music paused 🎵') } else { bgMusic.play(); setMusicOn(true); setDialogue('🎶 Enjoy the music!') }
  }, [musicOn])
  ...
  const handleZoneClick = useCallback((z) => { if (z.nav === '_music') toggleMusic(); else if (z.nav) { playNote(z.freq * 1.5, 0.8); go(z.nav) } else window.open('#', '_blank') }, [toggleMusic, go])
  const handleLetterHover = useCallback((e, i) => { playNote(261.6 * (1 + i * 0.1), 0.3); gsap.to(e.currentTarget, { y: 5, scaleY: 0.95, duration: 0.1, yoyo: true, repeat: 1 }) }, [])
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', overflow: 'hidden', background: '#E37B88' }}>
      ...
      <style>{`
        ...
        .social-icon { display: flex; align-items: center; justify-content: center; width: ${m ? 18 : 24}px; height: ${m ? 18 : 24}px; border-radius: 50%; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: rgba(255,255,255,0.7); cursor: pointer; transition: all 0.4s cubic-bezier(0.25,0,0,1); backdrop-filter: blur(8px); text-decoration: none; }
      ...
      <div className="hname" style={{ position: 'absolute', top: m ? '8%' : '20%', left: m ? 20 : 30, zIndex: 30, opacity: 0, transform: 'translateY(20px)' }}>
        {'Coco Choi'.split('').map((ch, i) => (
          <span key={i} className="piano-key-letter" style={{ fontFamily: "'Playfair Display', serif", fontSize: m ? 'clamp(24px, 8vw, 36px)' : 'clamp(40px, 9vw, 75px)', fontWeight: 900, color: 'white', display: 'inline-block' }}
            onMouseEnter={(e) => handleLetterHover(e, i)}>{ch === ' ' ? '\u00A0' : ch}</span>
        ))}
      </div>

      <div className="hsocials" style={{ position: 'absolute', bottom: m ? 20 : 40, left: m ? 20 : 30, zIndex: 30, display: 'flex', gap: m ? 5 : 8, opacity: 0 }}>
        {SOCIALS.map((s, i) => (<a key={i} className="social-icon" href={s.href} target="_blank" rel="noreferrer" onMouseEnter={() => playNote(880, 0.1)}>{SOCIAL_ICONS[i]}</a>))}
      </div>

      <div className="hsub2" style={{ position: 'absolute', top: m ? '18%' : '34%', right: m ? 20 : 40, left: m ? 20 : 'auto', zIndex: 30, opacity: 0 }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: m ? 'clamp(14px, 4vw, 20px)' : 'clamp(16px, 6vw, 32px)', fontWeight: 900, color: 'white', letterSpacing: -0.5, textAlign: m ? 'left' : 'right' }}>
          {displayed}<span style={{ borderRight: '2px solid white', animation: 'blink 0.7s infinite' }} />
        </div>
      </div>

      {m && (
        <div style={{ position: 'absolute', bottom: 60, right: 20, zIndex: 30, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[{ label: 'Experience', nav: 'experience' }, { label: 'Projects', nav: 'projects' }, { label: 'About Me', nav: 'about' }].map((item) => (
            <button key={item.nav} onClick={() => go(item.nav)} style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 20, padding: '6px 14px', color: 'white', fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer', backdropFilter: 'blur(8px)' }}>{item.label}</button>
          ))}
        </div>
      )}
