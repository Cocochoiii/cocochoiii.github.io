...
export default function AboutPage({ go }) {
  ...
  const handleCtaEnter = useCallback((e) => { e.currentTarget.style.transform = 'translateY(-4px) scale(1.03)'; e.currentTarget.style.boxShadow = `0 8px 25px ${EYE.rose}50` }, [])
  const handleCtaLeave = useCallback((e) => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = `0 4px 15px ${EYE.rose}40` }, [])
    ...
    <div ref={scrollRef} style={{ width: '100vw', height: '100vh', background: '#fff', position: 'relative', overflowX: 'hidden', overflowY: 'auto' }}>
      ...
      <div style={{ width: '100%', minHeight: m ? 'auto' : '100vh', position: 'relative', display: m ? 'flex' : 'block', flexDirection: 'column', alignItems: 'center', paddingTop: m ? 60 : 0, paddingBottom: m ? 40 : 0 }}>
        ...
        <div ref={rightRef} className="ab-right" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: m ? 4 : 6 }}>
            {ABOUT_STATS.map((st, i) => (
              <div key={st.label} style={{ position: 'relative', overflow: 'hidden', padding: m ? '8px 0' : '12px 0', borderRadius: 7, background: hoveredStat === i ? st.color : 'rgba(0,0,0,0.025)', border: `1.5px solid ${hoveredStat === i ? st.color : 'rgba(0,0,0,0.05)'}`, cursor: 'default', textAlign: 'center', transition: 'all 0.4s cubic-bezier(0.25,0,0,1)', boxShadow: hoveredStat === i ? `0 6px 15px ${st.color}30` : 'none' }}
                onMouseEnter={() => setHoveredStat(i)} onMouseLeave={() => setHoveredStat(null)}>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: m ? 16 : 22, fontWeight: 800, lineHeight: 1, color: hoveredStat === i ? '#fff' : st.color, transition: 'color 0.3s' }}>
                  <AnimatedNumber target={st.n} suffix={st.s} delay={0.8 + i * 0.2} />
                </div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 5 : 6, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: hoveredStat === i ? 'rgba(255,255,255,0.75)' : '#bbb', marginTop: 3, transition: 'color 0.3s' }}>{st.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ width: '100%', background: '#faf5f0', padding: m ? '30px 0' : 'clamp(40px, 10vw, 80px) 0', overflow: 'hidden' }}>
        ...
        <div className="ab-reveal" style={{ maxWidth: 550, margin: '0 auto', padding: pad, opacity: 0 }}>
          <div style={{ display: 'grid', gridTemplateColumns: m ? '1fr' : '1fr 1fr', gap: m ? 12 : 'clamp(20px, 5vw, 40px)' }}>
            <div>
              ...
              <div style={{ margin: m ? '10px 0' : '16px 0', height: 1, background: `linear-gradient(90deg, ${EYE.rose}40, transparent)` }} />
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 7 : 9, lineHeight: 2, color: '#666' }}>My professional journey includes building healthcare research tools at UbiWell Lab, designing IoT data pipelines at Audi, and developing enterprise web apps at Mars Inc. in Hong Kong.</p>
            </div>
            <div>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 7 : 9, lineHeight: 2, color: '#666' }}>I combine software engineering, design, and animation to create digital experiences that feel delightful to interact with. Outside of work, I founded Pet Paradise — a full-stack platform born from my love of caring for 20+ cats and dogs.</p>
              <div style={{ margin: m ? '10px 0' : '16px 0', height: 1, background: `linear-gradient(90deg, ${EYE.skin}40, transparent)` }} />
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 7 : 9, lineHeight: 2, color: '#666' }}>When I'm not coding, you'll find me drawing, exploring new design tools, or experimenting with Rive animations — like the ones you see on this very website.</p>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: m ? 'grid' : 'flex', gridTemplateColumns: '1fr 1fr', gap: m ? 10 : 0, justifyContent: m ? undefined : 'space-between', borderTop: m ? 'none' : `2px solid ${EYE.shadow}10`, paddingTop: m ? 0 : 20 }}>
            {ABOUT_BIG_STATS.map((s, i) => (
              <div key={i} style={{ textAlign: 'center', flex: m ? undefined : 1 }}>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: m ? 18 : 'clamp(16px, 3.5vw, 26px)', fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.num}</div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 7 : 7, letterSpacing: 1.5, textTransform: 'uppercase', color: '#aaa', marginTop: m ? 2 : 5, lineHeight: 1.4 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="ab-reveal" style={{ marginTop: m ? 20 : 40, opacity: 0 }}>
          <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', fontSize: m ? 5 : 7, color: EYE.skin, letterSpacing: 2, textTransform: 'uppercase', textAlign: 'center', marginBottom: m ? 8 : 14 }}>My Stack</p>
          <div style={{ overflow: 'hidden', position: 'relative' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: m ? 20 : 40, background: 'linear-gradient(90deg, #faf5f0, transparent)', zIndex: 2 }} />
            <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: m ? 20 : 40, background: 'linear-gradient(-90deg, #faf5f0, transparent)', zIndex: 2 }} />
        ...
        <div className="ab-reveal" style={{ maxWidth: 400, margin: '0 auto', opacity: 0 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: m ? 10 : 'clamp(15px, 4vw, 30px)', marginTop: m ? 18 : 28, flexWrap: 'wrap' }}>
            {CONTACT_INFO.map((c, i) => (
              <div key={i}>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 4 : 5, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', marginBottom: 3 }}>{c.label}</div>
                {c.href ? (
                  <a href={c.href} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 6 : 8, color: EYE.cream, textDecoration: 'none', fontWeight: 500, transition: 'color 0.3s' }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = EYE.rose)} onMouseLeave={(e) => (e.currentTarget.style.color = EYE.cream)}>{c.value}</a>
                ) : (
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: m ? 6 : 8, color: EYE.cream, fontWeight: 500 }}>{c.value}</div>
                )}
              </div>
            ))}
          </div>
