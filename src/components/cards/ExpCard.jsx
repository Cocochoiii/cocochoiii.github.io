...
function ExpCard({ exp, className = '', style = {} }) {
    ...
    const reveal = useCallback(() => {
        ...
        if (hoverRef.current) {
            gsap.fromTo(hoverRef.current,
                        ...
                        { clipPath: 'circle(150% at 50% 0%)', duration: 1.0, ease: 'power3.out' },
            )
        }
    }, [])
    ...
    const hide = useCallback(() => {
        ...
        if (hoverRef.current) gsap.to(hoverRef.current, { clipPath: 'circle(0% at 50% 0%)', duration: 0.7, ease: 'power2.in' })
    }, [])
