...
/**
 * Inner content for project cards.
 *
 * Font strategy:
 *   Patrick Hand  → project title (large), year watermark (decorative)
 *   DM Sans       → year label, tech meta (small, functional)
 */
function CardContent({ p, compact, hovered, isHover = false }) {
  ...
  const titleSize = m
                    ? (compact ? 9 : 12)
                    : (compact ? 'clamp(11px, 2.2vw, 16px)' : 'clamp(14px, 3vw, 22px)')
  ...
  return (
      <div style={{
        ...
        {/* Year watermark — Patrick Hand (decorative, large) */}
        <div style={{
        ...
        {/* Year label — DM Sans (tiny functional label) */}
        <div style={{
          position: 'absolute',
        ...
        {/* Title — Patrick Hand (large, handwritten) */}
        <h3 style={{
          fontFamily: "'Patrick Hand', cursive",
          fontSize: titleSize, fontWeight: 400,
          color: col, lineHeight: 1.1, letterSpacing: 0,
        ...
        {/* Meta — DM Sans (small tech labels, must be crisp) */}
        <div style={{
          fontFamily: "'DM Sans', sans-serif",
